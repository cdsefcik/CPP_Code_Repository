#include <iostream>
#pragma warning(disable : 4996) //Disables the error when using the fopen function which Visual Studio deems unsafe.
//#include <bits/stdc++.h> File used, this is a non standard library. I can access the needed function by importing the header from the standard libray.
#include "parser.h"
using namespace std;
/*
*This generates sql database tokens and open a database file.
* 
* 
*/
...
void writePhp(string file, string Data) {
    int n = Data.length();
    char char_array[n + 1];               //THIS IS AN ERROR, it is not evaluating to a constant.
...
void writePhp(string file, string Data) {
    ...
    char* char_array = new char[n + 1];   // FIX: dynamic allocation
    strcpy(char_array, Data.c_str());
...
void scanner(char* buffer) {
    string newf = "./new.php";                //Note, this might need to be set to windows paths.
    FILE* fpc = fopen(newf.c_str(), "w");
    fclose(fpc);

    newf = "./tokens.txt";                   //Note, this might need to be set to windows paths.
    ...
    while (!end_of_file()) {
        if (scan == true) {
            ...
            else
            {
                ...
                writePhp("./new.php", tmp_string); // new.php is the new generated file
...
int main(int argc, char** argv) //argv is filename to parse
{
    FILE* filePointer;
