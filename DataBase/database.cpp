//#include <bits/stdc++.h> //#include <bits/stdc++.h> File used, this is a non standard library. I can access the needed function by importing the header from the standard libray.
#ifndef __PARSER_H__
#include "parser.h"
#define __PARSER_H__
#endif
...
// used for multithreaded server project
...
void delete_row(string sql, string dbname) {
    ...
    MyReadFile.close();
    if (counter == 0) {
        error("table " + tablename + " does not exist", 49);
        return;
    }
    sql = remove_spaces(sql);
    valueStore = NULL;
...
void selects(string  sql, string  row, int pointer, string dbname) {
    ...
    if (getStr(sql, ' ') == "SELECT") {
        ...
        error("invalid select statement", 22);
        return;
    }
    sql = remove_space(sql);
    ...
    MyReadFisle.close();
    if (counter == 0) {
        error("table " + tablename + " does not exist", 50);
        return;
    }
    valueStore = NULL;
    while (sql != "") {
            ...
            if ((expect(TOKEN_IDENTIFIER, TOKEN_EQUAL, pointer)))
            {
                ...
                if ((expect(M_QUERY, TOKEN_LEFT_PAREN, pointer)))
                {
                    ...
                    if ((expect(TOKEN_RIGHT_PAREN, TOKEN_SEMICOLON, pointer))) {
                        ...
                        error("expected a closing parenthesis and semi colon", 0);
                        return pointer;
                    }
                }
                else
